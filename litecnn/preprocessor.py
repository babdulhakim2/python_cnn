...
class Preprocessor:
	...
	def squareTrim(M, min_side=20, threshold=0):
		...
		if (wend-wstart > hend-hstart):
			...
			shift = diff/2
			m[int(shift):int(cropped.shape[0]+shift),:cropped.shape[1]] = cropped
		else:
			...
			shift=diff/2
			# print("#"*20)
			# print(cropped.shape[1]+shift)
			m[:cropped.shape[0],int(shift):int(cropped.shape[1]+shift)] = cropped
		return m
	...
	def naiveInterp2D(M, newx, newy):
		...
		for i in range(M.shape[0]):
			for j in range(M.shape[1]):
				...
				# print("#"*20)
				# print(indx)
				result[int(indx),int(indy)] +=M[i,j]
		return result
